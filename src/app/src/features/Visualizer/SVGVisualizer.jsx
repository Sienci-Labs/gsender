...
class SVGVisualizer extends Component {
	...
	updateSVG() {
		...
		if (group && svg) {
			...
			let stroke = 1;
			const bbox = JSON.parse(JSON.stringify(reduxBBox)); // make shallow copy
			...
			const xLength = bbox.delta.x;
			const yLength = bbox.delta.y;

			// calculation for the middle of the svg
			const xMiddle = (bbox.min.x + bbox.max.x) / 2;
			const yMiddle = (bbox.min.y + bbox.max.y) / 2;
            ...
            */
			...
			const yZero =
				(yMiddle + (Math.abs(yLength) + Math.abs(yLength) / 2) / 2) * -1; // y is inversed because the svg starts off upside down
