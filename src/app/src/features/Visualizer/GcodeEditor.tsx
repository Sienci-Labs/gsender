...
const GcodeEditor = ({ onClose }: GcodeEditorProps) => {
	...
	useEffect(() => {
		if (isJobRunning && currentLineRunning >= 0 && scrollContainerRef.current) {
			...
			if (e.shiftKey && lastSelectedIndex !== null) {
				...
				if (newSelection.has(index)) {
					...
					if (lastSelectedIndex === index && newSelection.size > 0) {
						newLastSelectedIndex = Math.max(...Array.from(newSelection));
		...
		const handleKeyDown = (e: KeyboardEvent) => {
			...
			if ((e.ctrlKey || e.metaKey) && e.key === "f") {
				...
				const shouldDeleteLines =
					...
					(e.key === "Delete" && cursorPosition === inputValue.length);
	...
	const handleSearchNext = () => {
		...
		const nextIndex =
			currentMatchIndex < matchIndices.length - 1 ? currentMatchIndex + 1 : 0;
	...
	const handleSearchPrevious = () => {
		if (matchIndices.length === 0) return;
		const prevIndex =
			currentMatchIndex > 0 ? currentMatchIndex - 1 : matchIndices.length - 1;
	...
	const handleCopy = async () => {
		try {
			...
			if (selectedLines.size > 0) {
				...
				textToCopy = sortedIndices.map((index) => gcodeLines[index]).join("\n");
			} else {
	...
	return (
		<div className="z-[10000] w-full h-full flex flex-col bg-white dark:bg-dark shadow-lg rounded-md overflow-hidden dark:border dark:border-dark-lighter">
			<div className="flex justify-between items-center p-3 border-b border-gray-300 dark:border-dark-lighter gap-3">
				<div className="flex items-center gap-3 min-w-0 flex-1">
					...
					{isJobRunning && (
						...
						>
							{workflowState === WORKFLOW_STATE_PAUSED ? "Paused" : "Running"}
						</span>
					)}
					{selectedLines.size > 0 && (
			...
			<div className="flex gap-4 p-3 justify-end border-t border-gray-300 dark:border-dark-lighter">
				...
				<Button
					variant={
						selectedLines.size === gcodeLines.length ? "primary" : "outline"
					}
					size="icon"
