...
import type { SPINDLE_LASER_T } from "./definitions";
...
const SpindleWidget = () => {
	...
	const pendingSpindle =
		pendingSpindleIdRef.current !== null
			? (availableSpindles.find((s) => s.id === pendingSpindleIdRef.current) ??
				null)
			: null;
	const spindle =
		enabledSpindle ??
	...
	useEffect(() => {
		const tokens = [
			...
			pubsub.subscribe(
				"spindle:updated",
				(_: string, data: { spindleMax: number; spindleMin: number }) => {
					setState((prevState) => ({
						...prevState,
	...
	const getLaserOffsetCode = (preferredUnits: UNITS_GCODE | UNITS_EN) => {
		...
		const [xoffsetAdjusted, yOffsetAdjusted] = actions.calculateAdjustedOffsets(
			xOffset,
			yOffset,
			preferredUnits,
		);
	...
	const actions = {
		handleModeToggle: () => {
			const newMode =
				stateRef.current.mode === LASER_MODE ? SPINDLE_MODE : LASER_MODE;
		...
		sendLaserM3: () => {
			const laserPower =
				stateRef.current.laser.maxPower * (stateRef.current.laser.power / 100);
		...
		runShortcutLaserTest: () => {
			const spindle = get(reduxStore.getState(), "controller.spindles").find(
				(s) => s.enabled,
			) ?? {
				label: "Default Spindle",
				id: "0",
			...
			const laserMax = Number(
				get(reduxStore.getState(), "controller.settings.settings.$730") ?? 255,
			);
		...
		handleHALSpindleSelect: (selectedSpindle: {
			...
			const isModernFirmware = firmwarePastVersion(ATCI_SUPPORTED_VERSION);

			const spindleCommand = isModernFirmware ? "$spindlesh" : "$spindles";
