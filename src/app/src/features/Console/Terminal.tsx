...
const Terminal = (
	...
	const writeToTerminal = (data: string, source?: string) => {
		...
		if (source) {
			terminalInstance.current?.writeln(
				color.blackBright(source) + " " + color.xterm(TERMINAL_GREY)(data),
			);
			return;
		}
	...
	const controllerEvents = {
		"serialport:open": ({
			...
			terminalInstance.current?.writeln(
				color.white(`Connected to ${port} with a baud rate of ${baudrate}`),
			);
		...
		"serialport:read": (data: string) => {
			...
			if (isAnErrorMessage) {
				const throttledWriteToTerminal = throttle(writeToTerminal, 250, {
					trailing: false,
				});
