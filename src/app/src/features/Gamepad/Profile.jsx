...
const Profile = ({ data }) => {
	...
	const handleEditName = () => {
		...
		const updatedProfiles = profiles.map((profile) =>
			arrayComparator(profile.id, data.id) ? { ...profile, name } : profile,
		);
				...
				try {
					...
					const updatedProfiles = profiles.map((profile) =>
						arrayComparator(profile.id, data.id)
							? {
									...profile,
									buttons: importData.profile.buttons || profile.buttons,
									joystickOptions:
										importData.profile.joystickOptions ||
										profile.joystickOptions,
									lockout: importData.profile.lockout || profile.lockout,
									modifier: importData.profile.modifier || profile.modifier,
								}
							: profile,
					);
