...
const SetShortcut = () => {
	...
	const render = {
		category: (row) => {
			...
			return <div className={cn(baseClass, categoryClass)}>{row.category}</div>;
		},
		actions: (row) => {
			return (
							...
							<div className="border rounded-md max-h-[600px] overflow-y-auto">
								<Table>
									...
									<TableBody>
										{data.map((item, index) => (
											<TableRow key={index}>
												<TableCell>{render.category(item)}</TableCell>
												<TableCell>{render.actions(item)}</TableCell>
											</TableRow>
										))}
									</TableBody>
								</Table>
						...
						<div className="text-base">
							<div className="my-8">
								...
								<Switch
									...
									onChange={(checked) =>
										handleSetToggle("lockout", currentButtonValue, checked)
									}
								/>
							</div>
							...
							<div>
								...
								<Switch
									...
									onChange={(checked) =>
										handleSetToggle("modifier", currentButtonValue, checked)
									}
								/>
							</div>
						</div>
