	...
	useEffect(() => {
		...
		const gamepadListener = throttle(
			({ detail }) => {
				...
				if (isAvailable === AVAILABLE && inputRef.current.value === "") {
		...
		const output = {
			...
			[AVAILABLE]: (
				<div className="flex flex-col items-center gap-2 text-center p-4 text-green-600">
					...
					<small className="text-gray-500">Device ID: {gamepadInfo?.id}</small>
				</div>
			),
			[UNAVAILABLE]: (
		...
		<Dialog open={true} onOpenChange={() => closeModal()}>
			<DialogContent className="sm:max-w-md">
				...
				<div className="flex flex-col gap-4 p-4">
					...
					<Input ref={inputRef} placeholder="Enter a profile name here..." />
