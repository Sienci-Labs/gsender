...
describe('GcodeStepper controls', () => {
    it("steps forward and backward, clamped to the file's bounds", async () => {
        ...
        expect(screen.getByTestId('current-line-readout')).toHaveTextContent(
            '6',
        );

        await user.click(
            screen.getByRole('button', { name: /back 100 lines/i }),
        );
        expect(screen.getByTestId('current-line-readout')).toHaveTextContent(
            '1',
        );
    ...
    it('resets to line 1', async () => {
        ...
        await user.click(
            screen.getByRole('button', { name: 'Reset to start' }),
        );
        expect(screen.getByTestId('current-line-readout')).toHaveTextContent(
            '1',
        );
