...
const JobControl: React.FC<JobControlProps> = ({
	...
	const subscribe = () => {
		const tokens = [
			pubsub.subscribe("disconnect:recovery", (_msg: string, { received }) => {
				if (received) {
				...
				return [
					true,
					{
						...
						body: (
							<>
								<p>
									This file contains no tool change commands (M6) and the tool
									in the spindle will be used.
								</p>
								<p>Please confirm that you want to use this tool</p>
							</>
						),
					},
				];
				...
				return [
					true,
					{
						...
						body: (
							<>
								<p>
									The file contains no tool change commands (M6) and the tool in
									the spindle will be used. However, the tool in the spindle
									does not have an offset.
								</p>
								<p>
									Select <b>"Probe"</b> in the ATC tab to establish an offset
									and re-zero the workpiece before trying again.
								</p>
							</>
						),
					},
			...
			return [
				true,
				{
					...
					body: (
						<>
							<p>
								This file contains no tool change commands (M6) and there is no
								tool in the spindle.
							</p>
							<p>
								Load the tool you want to use into the spindle before trying
								again.
							</p>
							<p>
								Alternatively, you can update your post-processor to include a
								tool change command with your file.
							</p>
						</>
					),
				},
