...
export function ToolRemapDialog({
	...
	const currentIsManual = allowManualBadge && (currentTool?.isManual ?? false);
		...
		<Dialog open={open} onOpenChange={onOpenChange}>
			<DialogContent className="sm:max-w-[500px] gap-2">
				<DialogHeader>
					...
					<DialogDescription>
						Select a new tool number to remap T{originalTool}. Unavailable tools
						...
						<Select value={selectedTool} onValueChange={setSelectedTool}>
									...
									{selectedToolInfo ? (
										<div className="flex items-center gap-3 w-full flex-1 min-w-0">
											<span className="flex-1 min-w-0 font-mono font-medium truncate">
												{formatToolLabel(selectedToolInfo, selectedToolNumber)}
											</span>
											<ToolStatusBadges
												probeState={selectedToolInfo.status}
												isManual={selectedToolInfo.isManual && allowManualBadge}
												size="sm"
												className="ml-auto"
											/>
										</div>
							...
							<SelectContent className="z-[10000] bg-white dark:bg-dark">
								{allTools.map((tool) => {
									...
									const stateStyle = toolStateThemes[tool.status];
					...
					<div className="flex items-center justify-center gap-4">
						...
						<div className="flex items-center gap-2">
							...
							{getToolInfo(parseInt(selectedTool)) && (
								<ToolStatusBadges
									probeState={getToolInfo(parseInt(selectedTool))!.status}
									isManual={getToolInfo(parseInt(selectedTool))!.isManual}
									size="sm"
								/>
							)}
						</div>
