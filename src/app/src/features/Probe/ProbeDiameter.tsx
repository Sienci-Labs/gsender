...
const convertAvailableTools = (
	...
	return tools.map((tool) => ({
		value: String(
			tool[units === METRIC_UNITS ? "metricDiameter" : "imperialDiameter"],
		),
		label: String(
			tool[units === METRIC_UNITS ? "metricDiameter" : "imperialDiameter"],
		),
...
const ProbeDiameter = ({ actions, state, probeCommand }: Props) => {
	...
	useEffect(() => {
		const handleStoreChange = () => {
			...
			if (JSON.stringify(updatedTools) !== JSON.stringify(availableTools)) {
		...
		if (options.length === 0) {
			return (
				<div className="flex items-center justify-center h-full min-h-10">
					<p className="text-gray-500">No tools available, add one below.</p>
				</div>
			);
		}
		...
		return options.map((option) => (
			...
			>
				<div className="flex items-center justify-between w-full">
					<span>
						{option.label} {getUnitString(option.value as PROBE_TYPES_T)}
					</span>
				...
				<Popover>
					<PopoverTrigger asChild>
						...
						>
							{value ? (
								<span>
									{value} {getUnitString(value as PROBE_TYPES_T)}
								</span>
							) : (
								"Select diameter"
							)}
								...
								>
									<Input
										...
										onKeyDown={(e: KeyboardEvent<HTMLInputElement>) => {
											if (e.key === "Enter") {
												handleCreateOption(inputRef.current.value);
											}
										}}
										sizing="sm"
										ref={inputRef}
