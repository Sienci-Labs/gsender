...
import { TOUCHPLATE_TYPE_AUTOZERO, PROBE_TYPE_AUTO, PROBE_TYPE_TIP, PROBE_TYPE_DIAMETER } from 'app/lib/constants';

import { METRIC_UNITS } from '../../constants';
import { Actions, AvailableTool, ProbeCommand, State } from './definitions';
import { UNITS_EN } from 'app/definitions/general';
import { Select, SelectContent, SelectGroup, SelectItem, SelectTrigger, SelectValue } from 'app/components/shadcn/Select';
...
const ProbeDiameter: React.FC<Props> = ({ actions, state, probeCommand }) => {
    ...
    let { availableTools, units, toolDiameter, probeType } = state;
    const tools = [...availableTools];

    if (probeType === 'Tip' || probeType === 'Auto') {
        units = null;
    }
    ...
    return (
        <div className={cx("flex flex-row w-full", { "hidden": !probeCommand.tool })}>
            <Select
                onValueChange={handleChange}
                value={`${toolDiameter} ${units}`}
