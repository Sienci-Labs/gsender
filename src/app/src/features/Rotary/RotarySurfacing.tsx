...
const RotarySurfacing = () => {
	...
	const [surfacingState, setSurfacingState] = useState<RotarySurfacingOptions>(
		getInitialState(),
	);
	...
	function getInitialState(): RotarySurfacingOptions {
		const options = store.get("rotary.stockTurning.options", DEFAULT_VALUES_MM);
				...
				<div className="grid grid-cols-2 gap-4">
					<div className="grid gap-4 xl:gap-2">
						...
						<InputArea label="Stepdown">
							<Tooltip content={`Default is ${defaultValue.stepdown} ${units}`}>
								<ControlledInput
									id="stepdown"
						...
						<InputArea label="Spindle RPM">
							<Tooltip content={`Default is ${defaultValue.spindleRPM} RPM`}>
								<ControlledInput
									id="spindleRPM"
						...
						<InputArea label="Enable Rehoming">
							...
							<div className="flex flex-col gap-2 w-full col-span-2 text-xs xl:text-sm text-gray-500">
								Cut faster and cleaner by only rotating one direction, but you
								will need to rehome your A-axis at the end.
							</div>
						</InputArea>
					</div>
					<div className="flex flex-col border border-gray-200 rounded-md">
						...
						<div className="relative w-full h-full">
							...
							<div
								className={cx("h-full rounded-md relative p-2", {
									invisible: !tabSwitch,
								})}
							>
