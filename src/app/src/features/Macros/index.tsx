...
const MacroWidget = ({
	...
	const showToast = throttle(
		({ msg, type }: { msg: string; type: "info" | "success" | "error" }) => {
			toast[type](msg);
		},
		3000,
		{ trailing: false },
	...
	const exportMacros = () => {
		...
		const macrosClean = macros.map(({ name, content, description, id }) => ({
			name,
			content,
	...
	const importMacros = (e: React.ChangeEvent<HTMLInputElement>) => {
		...
		if (file) {
			...
			reader.onload = (event: ProgressEvent<FileReader>) => {
				const importedMacros = JSON.parse(event.target?.result as string);
				let importedCount = 0;
				let updatedCount = 0;

				for (const { id, name, content, description } of importedMacros) {
					if (name && content) {
	...
	return (
		...
		>
			{(modal.name === MODAL_ADD_MACRO || modal.name === MODAL_EDIT_MACRO) && (
				<MacroForm
					onSubmit={(data: {
						id?: string;
						name: string;
					...
					submitLabel={
						modal.name === MODAL_EDIT_MACRO ? "Update Macro" : "Add New Macro"
					}
				/>
			)}
