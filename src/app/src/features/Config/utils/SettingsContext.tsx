...
export function SettingsProvider({ children }: SettingsProviderProps) {
	const [settings, setSettings] = useState<SettingsMenuSection[]>(SettingsMenu);
	...
	function repopulateSettings() {
		const [populatedSettings, globalValues] = populateSettingsValues(settings);
		...
		debounce(
			(
				...
				const dirtyMap = new Map(
					currentEEPROM.filter((e) => e.dirty).map((e) => [e.setting, e]),
				);
				setEEPROM(
