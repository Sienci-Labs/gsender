...
describe("calculateHypotenuse", () => {
	...
	it("ignores the c value in calculation", () => {
...
describe("calculateAngle", () => {
	it("returns 0 for a perfect right triangle (3-4-5)", () => {
		// happy path
		const result = calculateAngle({ a: 3, b: 4, c: 5 });
		expect(result).toBeCloseTo(0, 1);
	});

	it("returns positive angle when diagonal is shorter than expected", () => {
		// happy path
		const result = calculateAngle({ a: 300, b: 300, c: 400 });
		expect(result).toBeGreaterThan(0);
	});
	it("returns NaN when null values are passed", () => {
		// failure path
	...
	it("returns negative angle when diagonal is longer than expected", () => {
		// happy path
		const result = calculateAngle({ a: 300, b: 300, c: 450 });
		expect(result).toBeLessThan(0);
	});

	it("returns near 0 for a nearly perfect square", () => {
		// edge case
		const a = 300;
		const b = 300;
	...
	it("returns near 0 for very small decimal values", () => {
		// edge case
		const a = 0.3;
		const b = 0.4;
		const c = Math.sqrt(a * a + b * b);
	...
	it("returns near 0 for decimal values", () => {
		// edge case
		const a = 5.5;
		const b = 6.6;
		const c = Math.sqrt(a * a + b * b);
	...
	it("returns a number for any valid triangle", () => {
		// edge case
	...
	it.skip("returns NaN for invalid triangle with negative sides", () => {
		// failure path
...
describe("determineEEPROMAdjustment", () => {
	...
	it("returns no adjustment needed when jog values are 0", () => {
		// edge case
	...
	it("returns no adjustment needed when triangle values are 0", () => {
		// edge case
		const result = determineEEPROMAdjustment(
			{ a: 0, b: 0, c: 0 },
			{ x: 300, y: 300, z: 0 },
	...
	it("returns current step values when null triangle and jog values are passed", () => {
		// failure path
		const result = determineEEPROMAdjustment(
	...
	it("returns current step value when no valid data", () => {
		// edge case
		const result = determineEEPROMAdjustment(
			{ a: 0, b: 0, c: 0 },
			{ x: 0, y: 0, z: 0 },
	...
	it("returns needsAdjustment true when moved distance differs from actual", () => {
		// happy path
		const result = determineEEPROMAdjustment(
			{ a: 290, b: 300, c: 424 },
			{ x: 300, y: 300, z: 0 },
	...
	it("returns needsAdjustment false when moved equals actual distance", () => {
		// happy path
		const result = determineEEPROMAdjustment(
			{ a: 300, b: 300, c: 424 },
			{ x: 300, y: 300, z: 0 },
	...
	it("calculates correct adjusted step value", () => {
		// happy path
	...
	it("handles different x and y step values from EEPROM", () => {
		// regression path
		mockGetState.mockReturnValue({
			controller: {
				settings: {
	...
	it("only adjusts x when only x data is valid", () => {
		// edge case
		const result = determineEEPROMAdjustment(
			{ a: 290, b: 0, c: 424 },
			{ x: 300, y: 0, z: 0 },
	...
	it("only adjusts y when only y data is valid", () => {
		// edge case
		const result = determineEEPROMAdjustment(
			{ a: 0, b: 290, c: 424 },
			{ x: 0, y: 300, z: 0 },
...
describe("calculateAngle + calculateHypotenuse integration", () => {
	it("machine is square when measured diagonal equals calculated hypotenuse", () => {
		// happy path
		const a = 300;
		const b = 300;
		const c = Math.sqrt(a * a + b * b);
	...
	it("machine is not square when diagonal is off", () => {
		// failure path
		const a = 300;
		const b = 300;
