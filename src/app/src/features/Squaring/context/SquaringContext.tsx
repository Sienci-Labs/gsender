import type { ButtonVariants } from "app/components/Button";
...
export const SquaringProvider = ({ children }: { children: ReactNode }) => {
				...
				step.shapeActions?.forEach((action) => {
					if (action.clearPrevious) {
						if (action.shapeType === "circlePoints") {
							updatedShapes.circlePoints = updatedShapes.circlePoints.map(
								(shape) => ({
									...shape,
									isActive:
										shape.id === action.shapeID ? action.isActive : false,
									show: shape.id === action.shapeID ? action.show : false,
								}),
							);
						} else if (action.shapeType === "arrows") {
							updatedShapes.arrows = updatedShapes.arrows.map((shape) => ({
								...shape,
								isActive: shape.id === action.shapeID ? action.isActive : false,
								show: shape.id === action.shapeID ? action.show : false,
							}));
						}
					} else {
	...
	const updateStepValue = (buttonLabel: string, value: number) => {
		setMainSteps((prevMainSteps) => {
			...
			currentMainStepData.subSteps = currentMainStepData.subSteps.map((step) =>
				step.buttonLabel === buttonLabel ? { ...step, value } : step,
			);
			return updatedMainSteps;
		});
	};
