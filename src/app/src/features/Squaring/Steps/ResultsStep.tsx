...
const ResultsStep = () => {
	...
	const hypotenuseDiff = Math.abs(calculatedHypotenuse - triangle.c).toFixed(2);
	...
	const needsEEPROMAdjustment =
		eepromAdjustment.x.needsAdjustment || eepromAdjustment.y.needsAdjustment;
		...
		if (isSquare) {
			return (
				<div className="text-green-950 bg-green-100 p-4 rounded-lg space-y-2">
					<p className="font-bold text-lg">Your machine is properly squared!</p>
			...
			return (
				<div className="text-yellow-800 bg-yellow-100 p-4 rounded-lg space-y-2 dark:bg-yellow-900 dark:text-white">
					...
					<p>
						...
						{units}) but you may want to adjust the right Y-axis rail to achieve
						better squareness.
					</p>
					...
					<p>
						...
						{hypotenuseDiff}
						{units} or the left y-axis rail backward by {hypotenuseDiff}
						{units}.
					</p>
				</div>
			);
						...
						<div className="space-y-1 text-yellow-800 bg-yellow-100 p-4 rounded-lg border min-h-52 flex flex-col justify-center items-start space-y-1 dark:bg-yellow-950 dark:text-white dark:border-yellow-950">
							...
							<div className="grid grid-cols-2 gap-2 mt-1">
								...
								<div className="p-2 bg-gray-50 rounded-lg dark:bg-dark">
									...
									<div className="text-xl font-bold text-blue-600">
										Recommended: {eepromAdjustment.y.amount.toFixed(3)}
									</div>
								</div>
							</div>
							<div className="flex flex-row gap-4">
									...
									<AlertDialog>
										<AlertDialogTrigger asChild>
											<Button disabled={isUpdating}>
												{isUpdating ? "Updating..." : "Update step/mm"}
											</Button>
										</AlertDialogTrigger>
										<AlertDialogContent className="bg-white">
											<AlertDialogHeader>
												...
												<div className="space-y-4">
													<p>
														This will update the X-axis ($100) and Y-axis ($101)
														step/mm values in your CNC firmware to the new ones
														below:
													</p>
													<p>
														X-axis:{" "}
								...
								<div className="mt-1 xl:mt-4 text-sm text-yellow-600 dark:text-yellow-400">
									...
									<p>
										Updating Firmware values can affect your machine's accuracy.
										Make sure to verify the new settings after applying them.
									</p>
								</div>
							</div>
						</div>
