...
const ErrorItem = ({ log, date, time }) => {
    return (
        <VerticalTimelineElement
            ...
            contentStyle={{
                height: 'auto',
                borderTop: `5px solid ${colorCodes[log.type]}`,
            }}
            contentArrowStyle={{ top: '42%' }}
            iconStyle={{
                top: '31%',
                background: colorCodes[log.type],
                color: '#fff',
            }}
        ...
        >
            ...
            <span className={styles.errorDate}>{`On ${date} at ${time}`}</span>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                <p>
                    {log.line &&
                        (log.source !== 'Console' && log.source !== 'Feeder'
                            ? `Line ${log.lineNumber}: `
                            : 'Line: ')}
                    {log.line}
                </p>
