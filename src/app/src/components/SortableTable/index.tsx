...
const SortableTable = <TData extends { subRow?: string }, TValue>(
	...
	const enableSortingRemoval =
		props.enableSortingRemoval !== undefined
			...
			: true;
	const searchPlaceholder = props.searchPlaceholder || "Search all columns...";
    ...
    */
	...
	return (
		<div className="w-full h-full flex flex-grow flex-col items-center gap-1">
			...
			{/*** GLOBAL SEARCH ***/}
			<div
				className={["flex items-center gap-2", styles.navContainer].join(" ")}
			...
			>
										...
										>
											{row.getVisibleCells().map((cell) => (
												<td
													...
													rowSpan={
														rowSpan.get(
															cell.column.columnDef.id, // id has replaced accessorKey
														) || 1
													}
													style={{
														whiteSpace: "unset",
				...
				{pagination && (
					...
					>
						<button
							className={cx("rounded border p-1 dark:text-white", {
								block: currentPage > 1,
						...
						>
							<span style={{ marginRight: "5px" }} className="dark:text-white">
								{"Jump to page: "}
								<input
									type="number"
							...
							<div style={{ marginRight: "5px" }} className="dark:text-white">
								|
							</div>
							{/*** PAGE SIZE ***/}
							<div>
								<span className="dark:text-white mr-1">Entries/Page:</span>
								<select
