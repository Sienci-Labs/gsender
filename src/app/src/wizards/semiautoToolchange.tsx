...
const getMoveAmount = () => {
	const $13 = get(
		reduxStore.getState(),
		"controller.settings.settings.$13",
		"0",
	);
				...
				substeps: [
					{
						...
						description: () => {
							const locationCopy =
								count === 1 ? "probe location" : "tool change location";

							return (
								<div>
									Ensure that your router/spindle is turned off and has fully
									stopped spinning then jog your machine to your {locationCopy}{" "}
				...
				substeps: [
					{
						...
						description: () => {
							...
							return (
								<div>
									{preamble} ({getToolString()}). Once ready, jog to{" "}
									{getMoveAmount()} above the probe location, attach the magnet,
									and prepare to probe
								</div>
							);
						},
						overlay: false,
