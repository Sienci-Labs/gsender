...
export const updateWorkspaceMode = (mode: WORKSPACE_MODE_T = WORKSPACE_MODE.DEFAULT): void => {
    ...
    const firmwareType: FIRMWARE_TYPES_T = get(reduxStore.getState(), 'controller.type');
    const rotaryFirmwareSettings: RotaryModeFirmwareSettings = store.get('workspace.rotaryAxis.firmwareSettings', ROTARY_MODE_FIRMWARE_SETTINGS);
    ...
    case DEFAULT: {
        if (firmwareType === GRBL) {
            const defaultFirmwareSettings: RotaryModeFirmwareSettings = store.get('workspace.rotaryAxis.defaultFirmwareSettings', DEFAULT_FIRMWARE_SETTINGS);
