...
class STLLoader {
	...
	load(url: string, onLoad: Function, onProgress: (event: ProgressEvent) => void, onError: (err: unknown) => void): void {
		let scope = this;
	...
	parse(data: string | ArrayBuffer) {
		const isBinary = (data: ArrayBufferLike): boolean => {
			let expect: number, face_size: number, n_faces: number, reader: DataView;
		...
		const matchDataViewAt = (query: Array<number>, reader: DataView, offset: number): boolean => {
		...
		const ensureBinary = (buffer: string | ArrayBuffer): ArrayBufferLike | Uint8Array => {
			if (typeof buffer === 'string') {
