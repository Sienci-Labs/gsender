...
const createTableRows = (
	...
	return Object.keys(array).map((key, index) => {
		...
		const rowStyle = index % 2 === 0 ? styles.tableRow : styles.tableRowEven;
		...
		return (
			<View
				...
				wrap={false}
			>
				<View style={[styles.tableCol, isDifferent && styles.highlightedCol]}>
					<Text
						style={[styles.tableCell, isDifferent && styles.highlightedText]}
					>
						{key}
					</Text>
				</View>
				...
				<View
					style={[styles.tableColLast, isDifferent && styles.highlightedCol]}
				>
					<Text
						style={[styles.tableCell, isDifferent && styles.highlightedText]}
					>
...
function generateSupportFile() {
		...
		<Document>
			<Page style={styles.body}>
				...
				<Text style={styles.author}>
					gSender v{version} • Generated on {new Date().toLocaleDateString()}
				</Text>
				...
				<View style={styles.grid}>
					<View style={styles.gridItem}>
						...
						<Text style={[styles.text, !isConnected && styles.statusDisabled]}>
							{isConnected
					...
					<View style={styles.gridItemLast}>
						...
						{machineProfile ? (
							<>
								...
								<Text style={styles.text}>{machineProfile.id}</Text>

								<Text style={styles.textBold}>Company:</Text>
								<Text style={styles.text}>{machineProfile.company}</Text>

								<Text style={styles.textBold}>Name:</Text>
								<Text style={styles.text}>{machineProfile.name}</Text>

								<Text style={styles.textBold}>Type:</Text>
								<Text style={styles.text}>{machineProfile.type}</Text>

								<Text style={styles.textBold}>Version:</Text>
								<Text style={styles.text}>{machineProfile.version}</Text>
								...
								<Text style={styles.textBold}>Spindle/Laser:</Text>
								<Text
									style={[
										styles.text,
										store.get("workspace.spindleFunctions", false)
											? styles.statusEnabled
											: styles.statusDisabled,
									]}
								>
								...
								<Text
									style={[
										styles.text,
										mode ? styles.statusEnabled : styles.statusDisabled,
									]}
								>
							...
							<Text style={styles.text}>No machine profile loaded</Text>
						)}
					</View>
				</View>
				...
				<View style={styles.grid}>
					<View style={styles.gridItem}>
						...
						{connection ? (
							<>
								<Text style={styles.textBold}>Connected Port:</Text>
								<Text style={styles.text}>
									{connection.port || "Not connected"}
								</Text>

								<Text style={styles.textBold}>Baudrate:</Text>
								<Text style={styles.text}>{connection.baudrate || "N/A"}</Text>

								{connection.port && (
									<>
										<Text style={styles.textBold}>Manufacturer:</Text>
										<Text style={styles.text}>
											{connection.manufacturer || "Unknown"}
										</Text>

										<Text style={styles.textBold}>In Use:</Text>
										<Text
											style={[
												styles.text,
												connection.inUse
								...
								<Text style={styles.textBold}>Available Ports:</Text>
								<Text style={styles.text}>
								...
								{connection.unrecognizedPorts &&
									connection.unrecognizedPorts.length > 0 && (
										<>
											<Text style={styles.textBold}>Unrecognized Ports:</Text>
											{connection.unrecognizedPorts.map((port, index) => (
					...
					<View style={styles.gridItemLast}>
						...
						{isConnected ? (
							<>
								<Text style={styles.textBold}>Type:</Text>
								<Text style={styles.text}>{grblInfo.type || "Unknown"}</Text>

								<Text style={styles.textBold}>Board:</Text>
								<Text style={styles.text}>{grblInfo.board || "Unknown"}</Text>

								<Text style={styles.textBold}>Firmware:</Text>
								<Text style={styles.text}>{firmwareVersion}</Text>
						...
						{isConnected && !isEmpty(grblInfo.mpos) && (
							<>
								<Text style={styles.textBold}>Machine Position:</Text>
								<Text style={styles.textSmall}>
						...
						{isConnected && !isEmpty(grblInfo.wpos) && (
							<>
								<Text style={styles.textBold}>Work Position:</Text>
								<Text style={styles.textSmall}>
				...
				<View style={styles.section}>
					...
					<View style={styles.grid}>
						<View style={styles.gridItem}>
							<Text style={styles.textBold}>Workspace Units:</Text>
							<Text
								style={[
									styles.text,
									workspaceUnits === "mm"
							...
							>
								{workspaceUnits === "mm" ? "Metric (mm)" : "Imperial (inches)"}
							</Text>
							...
							<Text
								style={[
									styles.text,
									isRotaryMode ? styles.statusEnabled : styles.statusDisabled,
								]}
							>
							...
							{isRotaryMode && (
								<>
									<Text style={styles.textBold}>Rotary Settings:</Text>
									{isConnected ? (
										<Text style={styles.textSmall}>
				...
				<View style={styles.section} break>
					...
					<Text
						style={[
							styles.text,
							fileStart?.enabled ? styles.statusEnabled : styles.statusWarning,
						]}
					>
					...
					{fileStart?.commands ? (
						<View style={styles.codeBlock}>
							<Text style={styles.codeBlockText}>{fileStart?.commands}</Text>
						</View>
					) : (
						<Text style={[styles.text, styles.addBottomMargin]}>{"N/A"}</Text>
					)}
					...
					<Text
						style={[
							styles.text,
							filePause?.enabled ? styles.statusEnabled : styles.statusWarning,
						]}
					>
					...
					{filePause?.commands ? (
						<View style={styles.codeBlock}>
							<Text style={styles.codeBlockText}>{filePause?.commands}</Text>
						</View>
					) : (
						<Text style={[styles.text, styles.addBottomMargin]}>{"N/A"}</Text>
					)}
					...
					<Text
						style={[
							styles.text,
							fileResume?.enabled ? styles.statusEnabled : styles.statusWarning,
						]}
					>
					...
					{fileResume?.commands ? (
						<View style={styles.codeBlock}>
							<Text style={styles.codeBlockText}>{fileResume?.commands}</Text>
						</View>
					) : (
						<Text style={[styles.text, styles.addBottomMargin]}>{"N/A"}</Text>
					)}
					...
					<Text
						style={[
							styles.text,
							fileStop?.enabled ? styles.statusEnabled : styles.statusWarning,
						]}
					>
					...
					{fileStop?.commands ? (
						<View style={styles.codeBlock}>
							<Text style={styles.codeBlockText}>{fileStop?.commands}</Text>
						</View>
					) : (
						<Text style={[styles.text, styles.addBottomMargin]}>{"N/A"}</Text>
					)}
						...
						<View style={styles.table}>
							...
							<View style={styles.tableHeader}>
								...
								<View style={styles.tableColLast}>
									<Text style={styles.tableCellHeader}>Default Value</Text>
								</View>
							</View>
							{/* TableContent */}
				...
				<View style={styles.section} break>
					...
					<View style={styles.grid}>
						<View style={styles.gridItem}>
							<Text style={styles.textBold}>All Alarms ({alarms.length})</Text>
							{alarms.length > 0 ? (
								...
								<Text style={styles.text}>No alarms recorded</Text>
							)}
						</View>

						<View style={styles.gridItemLast}>
							<Text style={styles.textBold}>All Errors ({errors.length})</Text>
							{errors.length > 0 ? (
								<View style={styles.container}>
									{errors.map((log) => (
										<View
				...
				<View style={styles.section}>
					...
					{fileInfo.fileLoaded && grblInfo.sender.status ? (
						<View>
							<Text style={styles.textBold}>File Information</Text>
							<Text style={styles.text}>
							...
							<Text style={styles.text}>
								Remaining: {grblInfo.sender.status.remainingTime}
							</Text>
							<Text style={styles.text}>
								Progress:{" "}
								<Text
							...
							<Text style={styles.textBold}>Full G-Code Content</Text>
							<View id="g-code-file-content" style={styles.codeBlock}>
								<Text style={styles.codeBlockText}>
									{gcode
	...
	return (
		<Button
			icon={
				<PiFileZipFill className="text-gray-600 w-8 h-8 dark:text-content-secondary" />
			}
			onClick={submitDiagnosticForm}
			size="lg"
			text={
				isGenerating ? progress || "Generating..." : "Download Diagnostic File"
			}
