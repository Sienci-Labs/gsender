...
class GCodeVirtualizer extends EventEmitter {
	...
	handlers: { [key: string]: (param: any) => void } = {
		...
		G2: (params: Record<string, any>): void => {
			...
			if (params.R) {
				...
				let height: number = Math.sqrt(4 * radius * radius - x * x - y * y) / 2;
	...
	calculateMachiningTime(endPos: BasicPosition, v1?: BasicPosition): void {
		...
		if (da !== 0 && (dx !== 0 || dy !== 0 || dz !== 0)) {
			...
			const programmedFeedMetric =
				this.modal.units === "G20" ? programmedFeed * 25.4 : programmedFeed;

			for (const axis of axisMovements) {
				let axisFeed =
					this.modal.motion === "G0" ? axis.maxFeed : programmedFeedMetric;
				...
				if (f > 0) {
					axisTime = this.getAcceleratedMove(axis.distance, f, axis.accel);
				}
	...
	updateSpindleToolEvents(word: SpindleToolEventCode, code: number | boolean) {
