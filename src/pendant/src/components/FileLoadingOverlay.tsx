...
export default function FileLoadingOverlay({ fileName, progress }: Props) {
    ...
    const shortName =
        fileName.length > 28 ? `…${fileName.slice(-26)}` : fileName;
    ...
    return (
        <div className="w-full min-h-[110px] rounded-xl border border-dashed border-outline-subtle bg-surface-sunken flex flex-col gap-3 p-4">
            ...
            <div className="flex items-center justify-between">
                <span className="font-mono text-xs text-content-muted truncate max-w-[70%]">
                    {shortName || 'loading…'}
                </span>
            ...
            <div className="flex flex-col gap-[3px]">
                {LINE_WIDTHS.map((maxWidth, i) => {
                    ...
                    const fillPct = Math.min(
                        maxWidth,
                        (lineProgress / (100 - i * 10)) * maxWidth,
                    );
                    return (
                        <div
                            key={i}
                            className="h-[5px] rounded-sm bg-surface-raised w-full"
                        >
                            <div
