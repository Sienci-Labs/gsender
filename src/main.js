    ...
    app.whenReady().then(async () => {
        try {
            ...
            ipcMain.on('logError:electron', (channel, error) => {
                ...
                (error.type === 'GRBL_ERROR') ? log.error(`GRBL_ERROR:Error ${error.code} - ${error.description} Line ${error.lineNumber}: "${error.line.trim()}" Origin- ${error.origin.trim()}`) : log.error(`GRBL_ALARM:Alarm ${error.code} - ${error.description}`);
            });
