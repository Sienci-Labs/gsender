...
const defaultMaintenance = [
	{
		...
		description:
			"Any buildup of dust or chips can block moving components and wear them out faster, making your machine less accurate and " +
	...
	},
	{
		...
		description:
			"Vibrations and wear during cutting can cause components to loosen over time. " +
	...
	},
	{
		...
		description:
			"Any components with bearings need occasional cleaning and lubrication to keep them running smoothly and prevent seizing from " +
	...
	},
	{
		...
		description:
			"Some CNC parts wear out over time and eventually need replacing - like lead screw nuts and v-wheels. " +
...
export function validateAndRepairConfigFile(file) {
	try {
		...
		try {
			...
			log.error(
				`Corrupt config "${file}" backed up to "${backupPath}" and reset to defaults`,
			);
...
class ConfigStore extends events.EventEmitter {
	...
	reload() {
		try {
			...
			log.error(
				`Unable to load data from ${chalk.yellow(JSON.stringify(this.file))}: err=${err}`,
			);
			this.emit("error", err); // emit error event
			return false;
		}
