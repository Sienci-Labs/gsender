...
const log: Log = logger("service:alarmStore") as Log;

class AlarmStore extends events.EventEmitter {
	file = "";
	configFile = "";
	...
	load(rcfile: string, errorFile: string) {
		this.file = errorFile;
	...
	reload() {
		try {
			if (fs.existsSync(this.file)) {
				...
				this.config = { alarmList: { list: [] } };
				const content = JSON.stringify({ alarmList: { list: [] } });
				...
				try {
					if (fs.existsSync(this.configFile)) {
						...
						// migrate alarms/errors into diff file
						if (fileConfig.alarmList) {
							try {
								// write alarms/errors to new file
								this.config = { alarmList: fileConfig.alarmList };
								const content = JSON.stringify({
									alarmList: fileConfig.alarmList,
								});
								fs.writeFileSync(this.file, content, "utf8");

								// delete alarms/errors from config file
								delete fileConfig.alarmList;
