...
export class YModem extends events.EventEmitter {
	...
	async sendFile(fileData, comms, progressCB) {
		...
		if (!fileData.data) {
			if (typeof fileData.content === "string") {
				console.log(
					`sendFile: converting content string to Buffer for "${fileData.name}"`,
				);
			...
			if (typeof fileData.content === "string") {
				...
				console.log(
					`sendFile: converting data string to Buffer for "${fileData.name}"`,
				);
		...
		const header = this.createHeaderPacket(
			this.SOH,
			fileData.name,
		...
		if (fileData.size === 0) {
			...
			fileChunks = [
				this.padRBuffer(fileData.data, SendSize[this.SOH], this.PAD_CHAR),
			];
			isLastByteSOH = true;
		} else if (fileData.size <= SendSize[this.STX]) {
			fileChunks = [
				this.padRBuffer(fileData.data, SendSize[this.STX], this.PAD_CHAR),
			];
			isLastByteSOH = false;
		} else {
		...
		let sendType =
			fileChunks.length === 1 && isLastByteSOH ? this.SOH : this.STX;
		...
		for (let packetNo = 1; packetNo <= fileChunks.length; packetNo++) {
			...
			await this.sendDataPacket(packetNo, dataPacket);
	...
	async sendFiles(files, comms) {
		...
		for (const fileData of files) {
			...
			if (!fileData.data) {
				if (typeof fileData.content === "string") {
					console.log(
						`sendFiles: converting content string to Buffer for "${fileData.name}"`,
					);
					fileData.data = Buffer.from(fileData.content, "utf8");
					fileData.size = fileData.data.byteLength;
				} else {
				...
				if (typeof fileData.content === "string") {
					...
					console.log(
						`sendFiles: converting data string to Buffer for "${fileData.name}"`,
					);
					fileData.data = Buffer.from(fileData.data, "utf8");
				} else {
					fileData.data = Buffer.from(fileData.data);
	...
	async sendDataPacket(packetNo, dataPacket) {
		...
		for (let retryCount = 1; retryCount <= 10; retryCount++) {
			...
			const waitForCCs = this.waitForNext([this.ACK, this.NAK, this.CAN]);
			...
			if (retryCount >= 9) {
				...
				throw new Error(`Packet timed out after ${retryCount} retries.`);
			}
		}
	}
	...
	waitForNextWithTimeout(controlChars, timeoutMs = 10000) {
		...
		const timeoutPromise = new Promise((_, reject) => {
			timeoutHandle = setTimeout(() => {
				reject(
					new Error(
						`Timeout waiting for control characters: ${controlChars.map((c) => DebugDict[c] || c).join(", ")}`,
					),
				);
			}, timeoutMs);
		});
