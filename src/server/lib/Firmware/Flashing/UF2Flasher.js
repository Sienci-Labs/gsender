...
class UF2Flasher extends events.EventEmitter {
	...
	async flash() {
		...
		try {
			...
			if (
				...
				log.info(
					`Volume disappeared during write (${err.code}) — treating as reboot`,
				);
			} else {
				this.emit(
					"error",
					`Failed to write UF2 to ${target}: ${err.message || err}`,
				);
				return;
			}
		}
	...
	findBootloaderVolume() {
		try {
			...
			log.error(
				`Error while searching for bootloader volume: ${err.message || err}`,
			);
			return null;
		}
	}
