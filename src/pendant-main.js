    ...
    app.whenReady().then(async () => {
        try {
            ...
            if (externalRendererUrl) {
                ...
                log.info(
                    `Pendant using external renderer URL in development: ${url}`,
                );
            } else if (process.env.NODE_ENV === 'development') {
                const errorMessage =
                    'ELECTRON_RENDERER_URL is required in pendant development mode';
                ...
                try {
                    ...
                    dialog.showMessageBoxSync(null, {
                        ...
                        message:
                            'gSender Pendant could not start its embedded server.',
                ...
                if (!(address && port)) {
                    log.error(
                        `Pendant server failed to bind: http://${address}:${port}`,
                    );
                    app.exit(-1);
                    return;
                }
