...
if (process.pkg) {
    ...
    function createSerialStub() {
        const unavailable = () =>
            new Error(
                'Serial port support unavailable in this build (native binding missing)',
            );
        return {
            open: (_p, _o, cb) =>
                cb ? cb(unavailable()) : Promise.reject(unavailable()),
            close: (_fd, cb) =>
                cb ? cb(unavailable()) : Promise.reject(unavailable()),
            read: (_fd, _b, _off, _len, _pos, cb) =>
                cb ? cb(unavailable()) : Promise.reject(unavailable()),
            write: (_fd, _b, cb) =>
                cb ? cb(unavailable()) : Promise.reject(unavailable()),
            drain: (_fd, cb) =>
    ...
    Module._load = function (request, parent, isMain) {
        if (request === 'node-gyp-build') {
            return function nodeGypBuildShim(dir) {
                ...
                const subdir =
                    plt === 'darwin' ? `${plt}-x64+arm64` : `${plt}-${arc}`;
                ...
                for (const name of [
                    ...
                    try {
                        return require(path.join(prebuildsDir, name));
                    } catch (_) {}
                }
                ...
                if (process.env.GSENDER_SERIALPORT_PREBUILDS) {
                    const externalDir = path.join(
                        process.env.GSENDER_SERIALPORT_PREBUILDS,
                        subdir,
                    );
                    try {
                        ...
                        return require(
                            path.join(externalDir, externalStem + '.node'),
                        );
                    } catch (_) {}
                    for (const name of [
                        'node.napi.node',
                        ...
                        try {
                            return require(path.join(externalDir, name));
                        } catch (_) {}
                    }
                }
                ...
                process.stderr.write(
                    '[sidecar] WARNING: serialport native binding unavailable — CNC serial connections will fail\n',
                );
