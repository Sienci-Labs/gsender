        ...
        <div className="rounded-xl bg-white border border-gray-200 dark:bg-dark-darker dark:border-dark-lighter p-4 flex flex-col gap-3">
            ...
            <div className="flex flex-col gap-2">
                {AXES.map(({ label, color }) => (
                    <div key={label} className="flex items-center gap-3 px-2 py-2 rounded-lg bg-gray-50 dark:bg-dark">
                        <span className={`w-8 h-8 rounded flex items-center justify-center text-sm font-bold shrink-0 ${color}`}>
                            {label}
                        </span>
                        <span className="flex-1 text-right font-mono text-4xl tabular-nums text-gray-900 dark:text-white leading-none">
                            0.000
                        </span>
            ...
            <div className="grid grid-cols-3 gap-2 pt-1">
                {[
                    ...
                    <button key={label} className="flex items-center justify-center gap-1.5 border border-gray-200 dark:border-dark-lighter rounded-lg py-3 text-sm text-gray-600 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-dark-lighter hover:text-gray-900 dark:hover:text-white">
                        <Icon size={14} />
                        {label}
                    </button>
                ))}
