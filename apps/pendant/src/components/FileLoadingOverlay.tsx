...
export default function FileLoadingOverlay({ fileName }: Props) {
    ...
    return (
        <div className="w-full min-h-[110px] rounded-xl border border-dashed border-dark-lighter bg-dark-darker flex flex-col gap-3 p-4">
            {/* Header */}
            <div className="flex items-center justify-between">
                <span className="font-mono text-xs text-[#6b8caa] truncate max-w-[70%]">{shortName || 'loading…'}</span>
                <span className="font-mono text-xs text-white tabular-nums">{pct}</span>
            </div>
            ...
            <div className="flex flex-col gap-[3px]">
                {LINE_WIDTHS.map((maxWidth, i) => {
                    ...
                    return (
                        <div key={i} className="h-[5px] rounded-sm bg-[#1e2d3d] w-full">
                            <div
            ...
            <div className="flex justify-end">
                <span className="text-[10px] text-[#6b8caa]">building toolpath...</span>
            </div>
        </div>
    );
}
