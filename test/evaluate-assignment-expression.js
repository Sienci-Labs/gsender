...
test("expressions", (t) => {
	{
		...
		evaluateAssignmentExpression(
			"  _x=(wposx+5), _y = wposy + 5, _z = (wposz+1*5)  ",
			vars,
		);
		t.same(vars, {
			wposx: 10,
...

	{
		...
		t.test((t) => {
			const vars = evaluateAssignmentExpression(
				'bar = "0", baz = 1, foo[bar][baz] = `${bar}${baz}`',
				{ bar, baz },
	...
	t.test((t) => {
		const vars = evaluateAssignmentExpression(
			"dx = 4000, dy = 3000, dz = 1000, object.volume = Number(dx * dy * dz) || 0",
			{ Number },
		);
	...
	// String
	t.test((t) => {
		const vars = evaluateAssignmentExpression("value = String(100)", {
			String,
		});
	...
	t.test((t) => {
		const vars = evaluateAssignmentExpression(
			"global.state.pos = JSON.stringify({ x: 0, y: 0, z: 0 })",
			{ JSON },
		);
	...
	t.test((t) => {
		const vars = evaluateAssignmentExpression(
			'binaryValue = parseInt("10000", 2), decimalValue = parseInt("325mm", 10)',
			{ parseInt },
		);
