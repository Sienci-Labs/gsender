	...
	return (
		<main className="app">
			<header>
				...
				<p className="lede">
					A manual test aid, not a real feature. Everything below should stay
					exactly as you left it while you switch tabs, resize the window, or
					navigate elsewhere in gSender and come back. If any of it resets, this
					tab's iframe just got torn down and rebuilt.
				</p>
			</header>
			...
			<section className="card counter">
				<h2>Mount count</h2>
				<p className="hint">
					Stored in this iframe's own <code>localStorage</code>, so it only
					survives a genuine reload of this tab — the only thing here that can
					prove a remount happened even if you weren't watching when it did.
				</p>
				<p className="big-number">{mountCount}</p>
				<p className="instruction">
					This number must not change unless you actually reload gSender. Switch
					to another tab and back, open Settings and return, resize the window —
					then check it's still the same.
				</p>
			...
			<section className="card">
				...
				<p className="hint">
					Plain React state — unlike the counter above, this is wiped instantly
					by a remount.
				</p>
				<input
			...
			<section className="card">
				...
				<p className="hint">
					...
					{loadedAt.current.toLocaleTimeString()}. A reset back to 0:00, or a
					jump in the timestamp above, means a remount just happened.
				</p>
				<ul className="log" aria-live="polite">
					{log.length === 0 && (
						<li className="log-empty">waiting for the first tick…</li>
					)}
					{log.map((line, index) => (
