describe('CNC Machine Tests Grbl', () => {
    ...
    it('Test Case: Connect to CNC and verify go to location', () => {
        ...
        cy.get('body > div:nth-of-type(2) input[type="number"]').then(
            ($inputs) => {
        ...
        cy.get('input[type="number"].text-xl.font-bold.text-blue-500.font-mono')
            ...
            .then(($inputs) => {
                ...
                cy.log(
                    `Current position: X=${xValue}, Y=${yValue}, Z=${zValue}`,
                );
                ...
                if (
                    ...
                    cy.log(
                        ' TEST PASSED: Machine is at home position (0.00, 0.00, 0.00) ',
                    );
                } else {
                    cy.log(' TEST FAILED: Machine is not at home position');
                    throw new Error(
                        `Expected position (0.00, 0.00, 0.00) but got (${xValue}, ${yValue}, ${zValue})`,
                    );
                }
            });
