...
beforeEach(() => {
    ...
    it('TC-UNLOCK-001: Unlock machine after connection when locked', () => {
        ...
        cy.get(
            'div.mt-4 button svg path, svg.hidden, button[aria-label*="Unlock"]',
        )
            .first()
            .parent('button')
            .should('be.visible')
    ...
    it('TC-UNLOCK-002: Verify unlock popup appears and can be dismissed', () => {
        ...
        cy.get('body').then(($body) => {
            ...
            if (unlockElements.length > 0) {
                ...
                cy.log(
                    'ℹ No unlock popup found - machine may already be unlocked',
                );
            }
        });
    ...
    it('TC-UNLOCK-004: Verify unlock button using exact selectors', () => {
        ...
        cy.get('body').then(($body) => {
            if (
                $body.find('#app > div > div.border div.mt-4 path').length > 0
            ) {
    ...
    it('TC-UNLOCK-005: Handle unlock error scenarios', () => {
        ...
        cy.get('body').then(($body) => {
            try {
                const unlockButton = $body.find(
                    'svg.hidden, button[aria-label*="Unlock"]',
                );
                ...
                if (unlockButton.length > 0) {
                    ...
                    cy.get('body').then(($newBody) => {
                        const stillLocked =
                            $newBody.find('svg.hidden').length > 0;
                        if (stillLocked) {
    ...
    it('TC-UNLOCK-007: Verify unlock button visibility and accessibility', () => {
        ...
        cy.get('body').then(($body) => {
            const unlockButton = $body
                .find('svg.hidden, button[aria-label*="Unlock"]')
                .first()
                .parent('button');
