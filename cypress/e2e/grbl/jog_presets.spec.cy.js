describe('Gsender - Update Rapid Presets & Verify Jogging Values', () => {
    ...
    it('Connects machine, updates rapid presets and verifies jogging values', () => {
        ...
        cy.log(
            'STEP 5 - VERIFY JOGGING SPEED DOES NOT EXCEED CONFIGURED LIMIT',
        );
        ...
        const verifyJog = (selector, label) => {
            ...
            cy.document().then((doc) => {
                const speedEl = doc.querySelector(
                ...
                const currentSpeed = speedMatch
                    ? parseInt(speedMatch[1], 10)
                    : 0;
                const maxSpeed = parseInt(EXPECTED_SPEED, 10);

                cy.log(
                    `${label} CURRENT SPEED: ${speedText} (${currentSpeed} mm/min)`,
                );
