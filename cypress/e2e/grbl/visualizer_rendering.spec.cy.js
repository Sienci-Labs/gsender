describe('CNC Machine File Info Test', () => {
    ...
    it('Connects to CNC, uploads file, and verifies visualizer + file info', () => {
        ...
        cy.log(
            ' Checking Outline and Start From buttons state BEFORE upload...',
        );

        cy.contains('button', 'Outline').then(($outlineBtn) => {
            if ($outlineBtn.length > 0) {
                const isDisabled =
                    $outlineBtn.is(':disabled') ||
                    $outlineBtn.hasClass('disabled');
                const isVisible = $outlineBtn.is(':visible');
                cy.log(
                    ` BEFORE upload - Outline button: exists=${true}, visible=${isVisible}, disabled=${isDisabled}`,
                );
            }
        });
        ...
        cy.contains('button', 'Start From').then(($startBtn) => {
            if ($startBtn.length > 0) {
                const isDisabled =
                    $startBtn.is(':disabled') || $startBtn.hasClass('disabled');
                const isVisible = $startBtn.is(':visible');
                cy.log(
                    ` BEFORE upload - Start From button: exists=${true}, visible=${isVisible}, disabled=${isDisabled}`,
                );
            }
        });
        ...
        cy.contains('button', 'Outline', { timeout: 10000 })
            ...
            .then(($btn) => {
                ...
                cy.wrap($btn)
                    ...
                    .then((parentId) => {
                        cy.log(
                            `Outline button parent container: ${parentId || 'no ID'}`,
                        );
                    });
            });
        ...
        cy.contains('button', 'Start From', { timeout: 10000 })
            ...
            .then(($btn) => {
                cy.log(
                    ' Start From button is visible and enabled after upload',
                );
                cy.wrap($btn)
                    .parents()
                    .first()
                    ...
                    .then((parentId) => {
                        cy.log(
                            ` Start From button parent container: ${parentId || 'no ID'}`,
                        );
                    });
            });
        ...
        cy.log(
            ' Result: File uploaded and visualized the file contents in the UI',
        );
        cy.log(' Outline button is visible and enabled after upload');
