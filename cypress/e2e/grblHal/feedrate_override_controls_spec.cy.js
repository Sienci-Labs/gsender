describe('Feedrate Performance Comparison Test', () => {
    ...
    it('Should compare job completion times with different feedrates', () => {
        ...
        cy.log(
            'Waiting for feedrate change to apply (acceleration/deceleration)...',
        );
        cy.wait(3000);
        ...
        cy.contains(/^Idle$/i, { timeout: 300000 })
            ...
            .then((status) => {
                cy.log(
                    `Machine status: "${status.text().trim()}" - First job completed`,
                );
            });
        cy.wait(2000);
        ...
        cy.contains(/^Idle$/i, { timeout: 300000 })
            ...
            .then((status) => {
                cy.log(
                    `Machine status: "${status.text().trim()}" - Second job completed`,
                );
            });
        cy.wait(2000);
        ...
        function parseTimeToSeconds(timeStr) {
            ...
            if (timeStr.includes(':')) {
                ...
                if (parts.length === 3) {
                    ...
                    cy.log(
                        `Parsed as HH:MM:SS: ${parts[0]}h ${parts[1]}m ${parts[2]}s`,
                    );
        ...
        cy.get('@firstJobInitialFeedrate').then((firstInitial) => {
            cy.get('@firstJobFeedrate').then((firstFeedrate) => {
                cy.get('@firstJobTime').then((firstTime) => {
                    cy.get('@secondJobInitialFeedrate').then(
                        (secondInitial) => {
                            cy.get('@secondJobFeedrate').then(
                                (secondFeedrate) => {
                                    cy.get('@secondJobTime').then(
                                        (secondTime) => {
                                            const firstSeconds =
                                                parseTimeToSeconds(firstTime);
                                            const secondSeconds =
                                                parseTimeToSeconds(secondTime);
                                            ...
                                            if (
                                                ...
                                                cy.log(
                                                    'ERROR: Failed to parse time values',
                                                );
                                                cy.log(
                                                    `First job time: "${firstTime}" -> ${firstSeconds}s`,
                                                );
                                                cy.log(
                                                    `Second job time: "${secondTime}" -> ${secondSeconds}s`,
                                                );
                                                return;
                                            }
                                            ...
                                            const diffMin = Math.floor(
                                                (timeDifference % 3600) / 60,
                                            );
                                            const diffSec = timeDifference % 60;

                                            let timeDiffStr = '';
                                            if (diffHours > 0)
                                                timeDiffStr += `${diffHours}h `;
                                            if (diffMin > 0)
                                                timeDiffStr += `${diffMin}m `;
                                            timeDiffStr += `${diffSec}s`;

                                            cy.log('');
                                            cy.log(
                                                'FIRST JOB (Reduced Feedrate):',
                                            );
                                            cy.log(
                                                `  Initial Feedrate: ${firstInitial}`,
                                            );
                                            cy.log(
                                                `  Final Feedrate: ${firstFeedrate}`,
                                            );
                                            cy.log(
                                                `  Time Taken: ${firstTime} (${firstSeconds} seconds)`,
                                            );
                                            cy.log('');
                                            cy.log(
                                                'SECOND JOB (Increased Feedrate):',
                                            );
                                            cy.log(
                                                `  Initial Feedrate: ${secondInitial}`,
                                            );
                                            cy.log(
                                                `  Final Feedrate: ${secondFeedrate}`,
                                            );
                                            cy.log(
                                                `  Time Taken: ${secondTime} (${secondSeconds} seconds)`,
                                            );
                                            cy.log('');
                                            ...
                                            if (secondSeconds < firstSeconds) {
                                                cy.log('CONCLUSION:');
                                                cy.log(
                                                    'SECOND JOB COMPLETED FIRST (Higher Feedrate)',
                                                );
                                                cy.log(
                                                    `  Feedrate: ${secondFeedrate}`,
                                                );
                                                cy.log(`  Time: ${secondTime}`);
                                                cy.log(
                                                    `  Faster by: ${timeDiffStr} (${percentageDiff}% improvement)`,
                                                );
                                            } else if (
                                                firstSeconds < secondSeconds
                                            ) {
                                                cy.log('CONCLUSION:');
                                                cy.log(
                                                    'FIRST JOB COMPLETED FIRST (Lower Feedrate)',
                                                );
                                                cy.log(
                                                    `  Feedrate: ${firstFeedrate}`,
                                                );
                                                cy.log(`  Time: ${firstTime}`);
                                                cy.log(
                                                    `  Slower by: ${timeDiffStr} (${percentageDiff}% slower)`,
                                                );
                                                cy.log(
                                                    '  Note: This is unexpected - higher feedrate should be faster',
                                                );
                                            } else {
                                                cy.log('CONCLUSION:');
                                                cy.log(
                                                    'Both jobs completed in exactly the same time',
                                                );
                                            }

                                            cy.log('');
                                            cy.log(
                                                'Feedrate performance comparison test completed successfully',
                                            );

                                            // Save comparison results to file
                                            const comparisonResults = {
                                                testName:
                                                    'Feedrate Performance Comparison',
                                                timestamp:
                                            ...
                                            cy.writeFile(
                                                'cypress/results/feedrate-comparison.json',
                                                comparisonResults,
                                            );
                                            cy.log(
                                                'Comparison results saved to: cypress/results/feedrate-comparison.json',
                                            );
                                        },
                                    );
                                },
