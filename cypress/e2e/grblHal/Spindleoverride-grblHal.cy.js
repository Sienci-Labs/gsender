describe("Spindle Configuration and Control Test - Decrease to 7500 RPM", () => {
	...
	it("Connect, Configure Spindle, Upload File, Decrease to 7500 RPM, Reset, Increase to 24000 RPM, and Complete Job", () => {
		...
		cy.get("fieldset:nth-of-type(1) span.sm\\:order-none button").click({
			force: true,
		});
		cy.wait(1000);
		...
		cy.get(
			"div.h-\\[25\\%\\] div.relative > div.h-full > div > div > div > div:nth-of-type(2) div:nth-of-type(2) > button",
		)
			.should("exist")
			.as("decreaseButton");
		...
		const decreaseUntilTarget = () => {
			cy.contains(/\d+\s*RPM/i, { timeout: 5000 })
				...
				.then((text) => {
					...
					cy.log(
						`Current RPM: ${currentSpindleRpm}, Target: ${targetSpindleRpm}, Clicks: ${decreaseClickCount}`,
					);
					...
					if (currentSpindleRpm > targetSpindleRpm) {
						...
						cy.get("@decreaseButton").click({ force: true });
						...
						cy.log(
							` Target RPM reached: ${currentSpindleRpm} RPM (after ${decreaseClickCount} clicks)`,
						);
					}
				});
		};
		...
		cy.get(
			"div.relative > div.h-full > div > div > div > div:nth-of-type(2) div:nth-of-type(1) > button",
		)
			.should("exist")
			.click({ force: true });
		...
		cy.get(
			"div.relative > div.h-full div:nth-of-type(2) > div.gap-2 > div:nth-of-type(3) > button",
		)
			.should("exist")
			.as("increaseButton");
		...
		const increaseUntilTarget = () => {
			cy.contains(/\d+\s*RPM/i, { timeout: 5000 })
				...
				.then((text) => {
					...
					cy.log(
						`Current RPM: ${currentSpindleRpm}, Target: ${targetIncreaseRpm}, Clicks: ${increaseClickCount}`,
					);
					...
					if (currentSpindleRpm < targetIncreaseRpm) {
						...
						cy.get("@increaseButton").click({ force: true });
						...
						cy.log(
							` Target RPM reached: ${currentSpindleRpm} RPM (after ${increaseClickCount} clicks)`,
						);
					}
				});
		};
		...
		cy.contains(/\d+\s*RPM/i)
			...
			.then((text) => {
				...
				if (increasedSpindleRpm >= 21600 && increasedSpindleRpm <= 26400) {
					...
					cy.log(
						` WARNING: Expected ~24000 RPM, got ${increasedSpindleRpm} RPM`,
					);
				}
