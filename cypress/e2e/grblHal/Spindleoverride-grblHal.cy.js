describe('Spindle Configuration and Control Test - Decrease to 7500 RPM', () => {
    ...
    it('Connect, Configure Spindle, Upload File, Decrease to 7500 RPM, Reset, Increase to 24000 RPM, and Complete Job', () => {
        ...
        const DECREASE_BTN = () =>
            cy
                .get(
                    'section div:nth-of-type(2) > div.gap-2 > div:nth-of-type(2) > button',
                )
                .first();
        const INCREASE_BTN = () =>
            cy
                .get(
                    'div.order-2 section > div > div > div > div:nth-of-type(2) div:nth-of-type(3) > button',
                )
                .first();
        const RESET_BTN = () =>
            cy
                .get(
                    'div.order-2 section > div > div > div > div:nth-of-type(2) div:nth-of-type(1) > button',
                )
                .first();
        ...
        cy.get('#fileInput').selectFile('cypress/fixtures/demo.gcode', {
            force: true,
        });
        cy.wait(5000);
        ...
        cy.get('#spindle-override')
            .parent()
            .then(($parent) => {
                const percentMatch = $parent.text().match(/(\d+)%/g);
                if (percentMatch) {
                    initialSpindleOverride =
                        percentMatch[percentMatch.length - 1];
                    cy.log(
                        'Initial Spindle Override: ' + initialSpindleOverride,
                    );
                }
            });
        ...
        Cypress._.times(5, (i) => {
            ...
            cy.contains(/\d+\s*RPM/i)
                ...
                .then((text) => {
                    ...
                    cy.log(
                        `Decrease click ${i + 1}/5 — Current RPM: ${currentSpindleRpm}`,
                    );
                });
        });
        ...
        cy.contains(/\d+\s*RPM/i)
            ...
            .then((text) => {
                ...
                cy.log(
                    'Spindle RPM after decrease: ' + reducedSpindleRpm + ' RPM',
                );
        ...
        cy.get('#spindle-override')
            ...
            .then(($parent) => {
                ...
                if (percentMatch) {
                    ...
                    cy.log(
                        'Spindle Override after decrease: ' + reducedOverride,
                    );
                }
            });
        cy.wait(2000);
        ...
        Cypress._.times(5, (i) => {
            ...
            cy.contains(/\d+\s*RPM/i)
                ...
                .then((text) => {
                    ...
                    currentSpindleRpm = match ? parseInt(match[1], 10) : null;
                    cy.log(
                        `Increase click ${i + 1}/5 — Current RPM: ${currentSpindleRpm}`,
                    );
                });
        });
        ...
        cy.get('#spindle-override')
            ...
            .then(($parent) => {
                ...
                if (percentMatch) {
                    ...
                    cy.log(
                        'Spindle Override after increase: ' + increasedOverride,
                    );
                }
            });
        ...
        // --------------------------------------------------------
        cy.log(
            'Step 19: Verify spindle returned to 100% override after increase',
        );
        cy.get('#spindle-override')
            .parent()
            .then(($parent) => {
                ...
                if (percentMatch) {
                    ...
                    cy.log(
                        'Spindle Override after second reset: ' +
                            secondResetOverride,
                    );
        ...
        cy.contains(/\d+\s*RPM/i)
            ...
            .then((text) => {
                ...
                cy.log(
                    'Spindle RPM after second reset: ' +
                        secondResetSpindleRpm +
                        ' RPM',
                );
                    ...
                    cy.log(
                        '  Total Duration: ' +
                            (jobEndTime && jobStartTime
                                ? ((jobEndTime - jobStartTime) / 1000).toFixed(
                                      2,
                                  )
                                : 'N/A') +
                            's',
        ...
        cy.get('@jobStatus').then((status) => {
            cy.get('@timeTaken').then((time) => {
                cy.get('@jobErrors').then((errors) => {
                    cy.writeFile(
                        'cypress/results/spindle-test-decrease-increase-stopped.json',
                        {
                            testName:
                                'Spindle Configuration and Control Test - Decrease & Increase (Stopped)',
                    ...
                    cy.log(
                        'Test details saved to: cypress/results/spindle-test-decrease-increase-stopped.json',
                    );
                });
            });
        });
