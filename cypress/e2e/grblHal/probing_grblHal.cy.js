describe('CNC Machine - Complete Z and XYZ Axis Probing', () => {
    ...
    it('Configures probe once, performs Z axis probing, then XYZ axis probing', () => {
        ...
        const checkProbeGreen = () => {
            return cy.get('body').then(($body) => {
                ...
                if ($probeTLS.length > 0) {
                    return (
                        $probeTLS.closest('.relative').find('.bg-green-500')
                            .length > 0
                    );
                }
                return false;
        ...
        const clickZMinus = (attempt = 1, maxAttempts = 30) => {
            ...
            cy.get('div.flex-row > div.flex path:nth-of-type(2)')
                .should('exist')
                .click({ force: true });
            cy.wait(1000);
        ...
        cy.get(
            'div.flex-shrink-0 > div > div > div > div > div.relative div:nth-of-type(3) input',
        )
            .invoke('val')
            .then((value) => {
            ...
            .then((value) => {
                ...
                cy.get('@initialZ0').then((initial) => {
                    ...
                    if (difference > 0.001) {
                        ...
                        throw new Error(
                            `Z0 did not change. Initial: ${initial}, Final: ${finalZ0}`,
                        );
                    }
                });
            });
