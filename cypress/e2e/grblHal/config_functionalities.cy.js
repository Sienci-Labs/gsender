describe("gSender Configuration and Firmware Test Suite", () => {
	...
	it("Should reset, export, import gSender settings, restore firmware defaults, and perform searches", () => {
				...
				.within(() => {
					cy.get('button[role="switch"]', { timeout: 10000 })
						...
						.then(($toggle) => {
							...
							expect(isMMSelected, "Units should be set to MM after import").to
								.be.true;
						});
				});
		});
		...
		cy.get(
			'[data-testid="firmware-settings-export-button"] > span.text-sm',
		).click();
		cy.wait(3000);
		cy.get("#section-0 fieldset:nth-of-type(3) > div:nth-of-type(1)").click({
			force: true,
		});

		// Import firmware settings
		cy.get(
			'[data-testid="firmware-settings-import-button"] > span.text-sm',
		).click();
		...
		cy.get("body").then(($body) => {
			if (
				$body.find('section:contains("EPROM settings imported")').length > 0
			) {
