    ...
    it('Verify Rapid preset values are updated and jogging speed/position is accurate', () => {
        ...
        const jogAndVerifySpeed = (label, selector) => {
            ...
            cy.wait(1500).then(() => {
                ...
                cy.get(
                    'div.h-\\[75\\%\\] section > div > div > div.gap-1 > div.items-center',
                )
                    .should('exist')
                    .then(($container) => {
                        ...
                        if (match) {
                            ...
                            expect(speedValue).to.equal(
                                EXPECTED_SPEED,
                                `${label}: Expected ${EXPECTED_SPEED} mm/min but got "${speedValue} mm/min"`,
                            );
                            cy.log(
                                `${label} speed verified: ${speedValue} mm/min`,
                            );
                        } else {
                            cy.log(
                                `WARNING: Could not extract speed from "${fullText}"`,
                            );
        ...
        cy.get('input[type="number"].h-6.text-sm')
            ...
            .then(($inputs) => {
                ...
                cy.log(
                    `Speed: Expected=${EXPECTED_SPEED}, Actual=${speedValue}`,
                );
                ...
                if (zValue !== EXPECTED_Z)
                    throw new Error(
                        `Z mismatch: Expected ${EXPECTED_Z}, got ${zValue}`,
                    );
        ...
        cy.get('[data-testid="wcs-input-X"]')
            .invoke('val')
            .then((x) => {
                cy.get('[data-testid="wcs-input-Y"]')
                    .invoke('val')
                    .then((y) => {
                        cy.get('[data-testid="wcs-input-Z"]')
                            .invoke('val')
                            .then((z) => {
                                cy.log(
                                    `Final position: X=${x}, Y=${y}, Z=${z}`,
                                );
                                if (
                                    x !== '0.00' ||
                                    y !== '0.00' ||
                                ...
                                cy.log(
                                    'Machine at home position (0.00, 0.00, 0.00)',
                                );
                            });
                    });
            });
