describe("Gsender testing preset create update", () => {
	...
	it("Test Case 1: Verify Rapid preset values are updated correctly", () => {
		...
		cy.contains("button", "Rapid").should("be.visible").click({ force: true });
		cy.wait(2000);
		...
		cy.get('input[type="number"].h-6.text-sm')
			...
			.then(($inputs) => {
				...
				if (xyValue === EXPECTED_XY) {
					...
					throw new Error(
						`XY mismatch: Expected ${EXPECTED_XY}, got ${xyValue}`,
					);
				}
				...
				if (speedValue === EXPECTED_SPEED) {
					...
					throw new Error(
						`Speed mismatch: Expected ${EXPECTED_SPEED}, got ${speedValue}`,
					);
				}
			});
	...
	it("Test Case 2: Verify jogging speed and position accuracy", () => {
		...
		const EXPECTED_SPEED = "4500";
		const speedVerificationResults = {
			xPlusYPlus: false,
			xMinusYMinus: false,
			zPlus: false,
			zMinus: false,
		...
		cy.get("body").then(() => {
			cy.document().then((doc) => {
				const speedElement = doc.querySelector(
					".min-w-4.text-center.text-blue-500",
				);
				if (speedElement && speedElement.textContent.includes("mm/min")) {
					...
					if (speedText.includes(EXPECTED_SPEED)) {
						...
						cy.log(
							`X+Y+ Speed mismatch: Expected ${EXPECTED_SPEED}, got ${speedText}`,
						);
					}
				}
			});
			...
			cy.document().then((doc) => {
				...
				if (speedElement && speedElement.textContent.includes("mm/min")) {
					...
					if (speedText.includes(EXPECTED_SPEED)) {
						...
						cy.log(
							`Z+ Speed mismatch: Expected ${EXPECTED_SPEED}, got ${speedText}`,
						);
					}
				}
			});
		...
		cy.get(
			...
			if (xValue === "0.00" && yValue === "0.00" && zValue === "0.00") {
				...
				throw new Error(
					`Position mismatch: Expected (0.00, 0.00, 0.00), got (${xValue}, ${yValue}, ${zValue})`,
				);
			}
		});
		...
		cy.wrap(null).then(() => {
			...
			if (allSpeedsVerified) {
				cy.log(
					` TEST CASE 2 PASSED: All jogging speeds verified at ${EXPECTED_SPEED} mm/min `,
				);
			} else {
				cy.log(
					"TEST CASE 2: Some speeds could not be verified (may be timing issue)",
				);
			}
		});
	});
