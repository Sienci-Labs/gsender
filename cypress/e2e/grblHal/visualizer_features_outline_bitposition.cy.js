describe('CNC Bit Position and Job Progress Tracking', () => {
    ...
    it('should connect, track bit positions, verify movement, and validate specific position values during job execution', () => {
        ...
        const capturePositions = (label) => {
            ...
            cy.get('input[type="number"].text-blue-500')
                .eq(0)
                .invoke('val')
                .then((xWork) => {
                    positionLog.work.x.push({
                        value: parseFloat(xWork),
                        label,
                    });
                });
            ...
            cy.get('input[type="number"].text-blue-500')
                ...
                .invoke('val')
                .then((yWork) => {
                    positionLog.work.y.push({
                        value: parseFloat(yWork),
                        label,
                    });
                });
            ...
            cy.get('input[type="number"].text-blue-500')
                ...
                .invoke('val')
                .then((zWork) => {
                    positionLog.work.z.push({
                        value: parseFloat(zWork),
                        label,
                    });
                });

            // Capture Machine Position values (X, Y spans)
            cy.get('span.font-mono.text-gray-400')
                .eq(0)
                .invoke('text')
                .then((xMachine) => {
                    positionLog.machine.x.push({
                        value: parseFloat(xMachine),
                        label,
                    });
                });
            ...
            cy.get('span.font-mono.text-gray-400')
                ...
                .invoke('text')
                .then((yMachine) => {
                    positionLog.machine.y.push({
                        value: parseFloat(yMachine),
                        label,
                    });
                });

            positionLog.timestamps.push({
        ...
        // Zero Y axis
        cy.contains('button', 'Y0').should('not.be.disabled').click();
        cy.wait(500);

        // Zero Z axis
        cy.contains('button', 'Z0').should('not.be.disabled').click();
        cy.wait(1000);
        ...
        monitoringIntervals.forEach((interval, index) => {
            cy.wait(
                interval - (index > 0 ? monitoringIntervals[index - 1] : 0),
            );
        ...
        cy.then(() => {
            ...
            cy.log(
                `Machine X positions: ${JSON.stringify(positionLog.machine.x)}`,
            );
            cy.log(
                `Machine Y positions: ${JSON.stringify(positionLog.machine.y)}`,
            );
            ...
            cy.log(
                ' Position data saved to: cypress/results/position-log.json',
            );
            ...
            const movementSummary = {
                ...
                positions: {
                    ...
                    final: {
                        x:
                            positionLog.work.x[positionLog.work.x.length - 1]
                                ?.value || 0,
                        y:
                            positionLog.work.y[positionLog.work.y.length - 1]
                                ?.value || 0,
                        z:
                            positionLog.work.z[positionLog.work.z.length - 1]
                                ?.value || 0,
                    },
                },
            };
            ...
            cy.writeFile(
                'cypress/results/movement-summary.json',
                movementSummary,
            );
            cy.log(
                ' Movement summary saved to: cypress/results/movement-summary.json',
            );
        });
