describe("Surfacing Tool - Input Fields", () => {
	...
	it("should navigate to Surfacing and fill in all input fields", () => {
		...
		// Width
		cy.get("#width").clear().type("75");

		// Length
		cy.get("#length").clear().type("75");

		// Skim Depth
		cy.get("#skimDepth").clear().type("1");

		// Max Depth
		cy.get("#maxDepth").clear().type("1");

		// Bit diameter (4th field — no ID, use positional selector)
		cy.get("div.px-8 div:nth-of-type(4) input").clear().type("22");

		// Stepover (5th field)
		cy.get("div.px-8 div:nth-of-type(5) input").clear().type("40");

		// Feed rate (6th field)
		cy.get("div:nth-of-type(6) input").clear().type("2500");

		// Spindle RPM (7th field)
		cy.get("div:nth-of-type(7) input").clear().type("17000");
