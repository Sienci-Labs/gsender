describe('Macros Upload, Execution and Verification Test', () => {
    ...
    const deleteAllMacros = () => {
        cy.get('body').then(($body) => {
            ...
            const menuButtons = $body.find(
                'button.flex.items-center.justify-center.w-10.h-8.cursor-pointer[aria-haspopup="menu"]',
            );
            ...
            cy.wrap(menuButtons[0]).scrollIntoView().click({ force: true });
            ...
            cy.get(
                'div[role="menuitem"].cursor-pointer, [class*="cursor-pointer"][class*="px-4"][class*="py-3"]',
            )
                .contains(/Delete/i)
                .first()
                .should('be.visible')
    ...
    it('Upload and run macro with success message verify the output', () => {
        ...
        cy.get('input[type="file"]').then(($inputs) => {
            if ($inputs.length > 1) {
                cy.log(
                    `Multiple file inputs found (${$inputs.length}), using the last one`,
                );
                cy.wrap($inputs.last()).selectFile(
                    'cypress/fixtures/macrosample.json',
                    { force: true },
                );
            } else {
                cy.wrap($inputs.first()).selectFile(
                    'cypress/fixtures/macrosample.json',
                    { force: true },
                );
            }
        ...
        cy.get('section ol li', { timeout: 10000 })
            ...
            .then(($toast) => {
                ...
                const hasSuccessMessage =
                    /uploaded|imported|success|added|macro/i.test(
        ...
        cy.get('section ol li', { timeout: 15000 })
            ...
            .then(($toast) => {
                ...
                const hasExecutionMessage =
                    /running|executed|started|success|completed|macro/i.test(
                        toastText.toLowerCase(),
                    );
                if (hasExecutionMessage) {
                    ...
                    cy.log(
                        `Warning: Unexpected execution message: ${toastText}`,
                    );
                }
            });
        ...
        cy.get('textarea[name="description"][maxlength="128"]', {
            timeout: 10000,
        })
            .should('be.visible')
            .clear({ force: true })
        ...
        cy.wait(1000);
        cy.get('[data-testid="add-macro-button"]').click({ force: true });
        cy.wait(2000);
            ...
            if ($body.find('section ol li').length > 0) {
                cy.get('section ol li', { timeout: 10000 })
                    ...
                    .then(($toast) => {
                        ...
                        const hasSuccess = /added|success|created|macro/i.test(
                            toastText,
                        );
