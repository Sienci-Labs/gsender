describe('CNC Visualizer rendering test case', () => {
    ...
    it('Connects to CNC, uploads file, and verifies visualizer + file info', () => {
        ...
        cy.get('body').then(($body) => {
            if (
                $body.find('#app > div > div.border div.mt-4 button').length > 0
            ) {
                cy.log('Unlock popup found - clicking to unlock...');
                cy.get('#app > div > div.border div.mt-4 button').click({
                    force: true,
                });
                cy.wait(2000);
        ...
        cy.log(
            'Step 4: Checking Outline and Start From buttons state BEFORE upload...',
        );
