describe('Dynamic Keyboard Jogging Test - All Axes', () => {
        ...
        cy.get('tbody tr', { timeout: 10000 }).then(($rows) => {
            ...
            $rows.each((index, row) => {
                ...
                if (searchPattern.test(rowText)) {
                    ...
                    cy.wrap(row).within(() => {
                        cy.get(
                            'td:nth-of-type(2) > div > div:nth-of-type(2) > div:nth-of-type(2) svg',
                        ).click();
                    });
                    foundRow = true;
                    return false;
            ...
            if (!foundRow)
                throw new Error(
                    `Could not find row matching: ${searchPattern}`,
                );
        });
                ...
                .then((startPos) => {
                    ...
                    cy.get(selector)
                        ...
                        .then((endPos) => {
                            ...
                            cy.log(
                                `Final: ${finalPos}, Difference: ${difference.toFixed(4)} mm`,
                            );
    ...
    it('Test all keyboard jog shortcuts dynamically', () => {
        ...
        const goToKeyboardShortcuts = () => {
            cy.get(
                'div.flex > div.flex > div.flex a:nth-of-type(3) svg',
            ).click();
            cy.wait(2000);
