describe("A-Axis Rotary operations", () => {
	...
	it("Tests A-axis go to location functionality with positive values", () => {
		...
		cy.contains("button", "Rotary").filter(":visible").click();
		cy.wait(500);
		...
		cy.get('button[role="switch"]').then(($toggle) => {
		...
		cy.contains("button", "Mounting Setup").filter(":visible").click();
		cy.wait(500);
		...
		cy.get(
			'[id^="radix-"] > div.justify-between > div:nth-of-type(1) div:nth-of-type(1) > button',
		)
			.filter(":visible")
			.click();
		cy.wait(300);
		...
		cy.get(
			'[id^="radix-"] > div.justify-between > div:nth-of-type(2) div:nth-of-type(2) > button',
		)
			.filter(":visible")
			.click();
		cy.wait(300);
		...
		cy.contains("button", "Load G-Code to").filter(":visible").click();
		cy.wait(1000);
		...
		cy.get("div.block > div.block > div > div.flex button")
			...
			.then(($toggle) => {
				cy.log(
					`  Rotary toggle current state: ${$toggle.attr("aria-checked") ?? "unknown"}`,
				);
				if (
					$toggle.attr("aria-checked") === "false" ||
					$toggle.attr("aria-checked") === undefined
				) {
					cy.wrap($toggle).click();
					cy.wait(300);
		...
		cy.get("div.min-h-10 > div:nth-of-type(1) > button").click();
		cy.wait(500);
		...
		cy.get("body > div:nth-of-type(2) div:nth-of-type(5) input").should(
			"have.value",
			"5",
		);
		...
		cy.get('[data-testid="wcs-input-Y"]:not([disabled])').should(
			"have.value",
			"5.00",
		);
