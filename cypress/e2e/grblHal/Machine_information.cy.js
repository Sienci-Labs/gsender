    ...
    it('Checks machine information, verifies pin inversion, and tests stepper motor lock/unlock', () => {
        ...
        const checkLimitState = (limitName) => {
            return cy.get('body > div:nth-of-type(2)').then(($popup) => {
                ...
                if ($rows.length === 0) {
                    cy.log(
                        `WARNING: Could not find label "${limitName}" in popup`,
                    );
                    return 'not-found';
                }
                ...
                cy.log(
                    `Unknown path near "${limitName}": ${pathD.substring(0, 40)}`,
                );
                return 'unknown';
            });
        };
        ...
        const applyIfNeeded = () => {
            cy.get('body').then(($body) => {
                ...
                if ($btn.length > 0 && !$btn.is(':disabled')) {
                    ...
                    cy.log(
                        'Apply Settings not needed - Settings already up to date',
                    );
                }
            });
        };
        ...
        cy.get('div.mt-4 > button').should('be.visible').as('stepperToggle');
        ...
        cy.get('@stepperToggle').then(($toggle) => {
            const isLocked =
                $toggle.attr('data-state') === 'checked' ||
                $toggle.attr('aria-checked') === 'true';
            expect(isLocked, 'Stepper motor should be UNLOCKED at test end').to
                .be.false;
