...
Cypress.Commands.add('ensureHomingEnabledAndHome', (options = {}) => {
    ...
    cy.get('button#\\$22-0-key').then(($toggle) => {
        ...
        if (verifyHomingStatus) {
            ...
            cy.contains('span', 'Homing', { timeout: 10000 }).should(
                'be.visible',
            );
    ...
    cy.contains(/^Idle$/i, { timeout: 20000 }).then((status) => {
        ...
        if (machineStatus.toLowerCase() === 'idle') {
            ...
            cy.contains(/Connect to CNC/i, { timeout: 30000 })
                .should('be.visible')
                .then(() =>
                    cy.log('Machine disconnect verified successfully.'),
                );
        } else {
    ...
    (expectedX = 0, expectedY = 0, expectedZ = 0) => {
        ...
        cy.get('input[type="number"].text-xl.font-bold.text-blue-500.font-mono')
            ...
            .then(($inputs) => {
                ...
                cy.log(
                    `Current positions - X: ${xValue}, Y: ${yValue}, Z: ${zValue}`,
                );
                cy.log(
...
Cypress.Commands.add('checkProbingIsActive', (options = {}) => {
    ...
    const checkGreen = () => {
        return cy.get('body').then(($body) => {
            const $probeTLS = $body.find(
                'div.text-gray-500:contains("Probe/TLS")',
            );
...
Cypress.Commands.add('verifyMachineStatus', (expectedStatus, options = {}) => {
    ...
    cy.contains(statusRegex, { timeout })
        ...
        .then(($el) => {
            ...
            expect(actualStatus.toLowerCase()).to.eq(
...
Cypress.Commands.add('loadUI', (options = {}) => {
    const { timeout = 20000, viewport = { width: 1920, height: 1080 } } =
        options;
