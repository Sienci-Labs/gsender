...
const CYPRESS_REPORT =
    POSSIBLE_REPORTS.find((p) => fs.existsSync(p)) || POSSIBLE_REPORTS[0];
    ...
    try {
        ...
        function walkSuites(suites, specFile) {
            ...
            suites.forEach((suite) => {
                const name =
                    specFile || suite.file || suite.title || 'Unknown Spec';
                if (suite.tests && suite.tests.length) {
                    suite.tests.forEach((test) => {
                        allTests.push({
                            ...
                            state:
                                test.state || (test.fail ? 'failed' : 'passed'),
                            error:
        ...
        cypress = {
            date: stats.end
                ? new Date(stats.end).toLocaleDateString('en-CA', {
                      year: 'numeric',
                      month: 'long',
                      day: 'numeric',
                  })
            ...
            passRate:
                stats.tests > 0
                    ? Math.round((stats.passes / stats.tests) * 100)
                    : 0,
            failures,
            bySpec,
        };
...
function calc(mod) {
    ...
    return {
        ...
        pct: total > 0 ? Math.round((created / total) * 100) : 0,
    };
}
const mods = data.modules.map((m) => ({ ...m, ...calc(m) }));
const g = mods.reduce(
    (a, m) => ({ total: a.total + m.total, created: a.created + m.created }),
    { total: 0, created: 0 },
);
...
function buildDetailedTestResults() {
    ...
    let html =
        '<div class="ts-grid">' +
        '<div class="ts-card"><div class="ts-label">Total Tests</div><div class="ts-val">' +
        cypress.total +
        '</div></div>' +
        '<div class="ts-card passed"><div class="ts-label">Passed</div><div class="ts-val tpass">' +
        cypress.passed +
        '</div></div>' +
        '<div class="ts-card failed"><div class="ts-label">Failed</div><div class="ts-val tfail">' +
        cypress.failed +
        '</div></div>' +
        '<div class="ts-card skip"><div class="ts-label">Pending</div><div class="ts-val tskip">' +
        cypress.pending +
        '</div></div>' +
        '<div class="ts-card"><div class="ts-label">Duration</div><div class="ts-val tdur">' +
        cypress.duration +
        '</div></div>' +
        '<div class="ts-card"><div class="ts-label">Pass Rate</div><div class="ts-val" style="color:' +
        prColor +
        '">' +
        cypress.passRate +
        '%</div></div>' +
    ...
    html +=
        '<div class="pr-wrap"><div class="pr-labels"><span>Pass Rate</span><span style="color:' +
        prColor +
        ';font-weight:700">' +
        cypress.passRate +
        '%</span></div><div class="pr-bg"><div class="pr-fill" style="width:' +
        cypress.passRate +
        '%;background:' +
        prColor +
        '"></div></div></div>';
    ...
    Object.keys(cypress.bySpec).forEach((spec) => {
        ...
        html +=
            '<div class="spec-block"><div class="spec-header">' +
            ...
            (passed
                ? '<span class="sc-pass">✔ ' + passed + ' passed</span>'
                : '') +
            (failed
                ? '<span class="sc-fail">✘ ' + failed + ' failed</span>'
                : '') +
            (pending
        ...
        tests.forEach((t) => {
            const icon =
                t.state === 'passed' ? '✔' : t.state === 'pending' ? '⏭' : '✘';
            const rowClass =
                t.state === 'passed'
                    ? 'tr-pass'
                    : t.state === 'pending'
            ...
            if (t.state === 'failed' && t.error) {
                html +=
                    '<div class="tr-error"><div class="fail-error">' +
                    t.error.replace(/</g, '&lt;').replace(/>/g, '&gt;') +
                    '</div>' +
                    (t.stack
                        ? '<div class="fail-stack">' +
                          t.stack.replace(/</g, '&lt;').replace(/>/g, '&gt;') +
                          '</div>'
                        : '') +
                    '</div>';
            }
...
function moduleCard(m) {
    ...
    return (
        '<div class="mc"><div class="mh" style="border-left:4px solid ' +
        c.bar +
        '"><div><div class="mn">' +
        m.name +
        '</div><div class="mm">' +
        ...
        ' of ' +
        m.total +
        ' test cases created</div></div><div class="mb" style="color:' +
        c.pct +
        ';border-color:' +
        c.bd +
        ';background:' +
        ...
        '">' +
        m.pct +
        '%</div></div><div class="mpr"><span class="plb">Coverage</span><div class="pb"><div class="pf" style="width:' +
        m.pct +
        '%;background:' +
        c.bar +
        '"></div></div><span class="pp" style="color:' +
        c.pct +
        '">' +
        m.pct +
        '%</span></div><div>' +
        featureRows(m.features) +
        '</div></div>'
    );
}
